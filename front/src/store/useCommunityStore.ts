...
export const useCommunityStore = create<CommunityStoreState>((set, get) => ({
	...
	getCommunities: async (query: string) => {
		...
		try {
			const res = await axiosInstance.get(
				`/community/search?name=${query}&tagName=${query}`,
			);
	...
	getCommunityMessages: async (id: string) => {
		...
		try {
			const res = await axiosInstance.get(
				`/community/${id}/messages?page=${PAGE}&limit=${LIMIT}`,
			);
	...
	sendCommunityMessages: async ({ image, text }: SendMsgProps) => {
		...
		try {
			const res = await axiosInstance(
				`community/send-message/${selectedCommunity?._id}`,
				{
					method: "POST",
					data: { image, text },
				},
			);
			const messageWithMySender = {
				...res.data.newMessage,
				senderId: {
					_id: me?._id,
			...
			set({
				communityMessages: [...(communityMessages || []), messageWithMySender],
			});
		} catch (error) {
			console.log(error);
			if (error instanceof AxiosError) {
	...
	setCommunitySearchResults: (communities: Community[] | undefined) =>
		set({ communitySearchResults: communities }),
	...
	subscribeToCommunityMessages: () => {
		...
		socket.on("newCommunityMessage", (data) => {
			...
			const { communityMessages, selectedCommunity, unreadCommunityCounts } =
				get();
	...
	requestTojoinCommunity: async (communityId: string) => {
		...
		try {
			const res = await axiosInstance.post(
				`community/join-request/${communityId}`,
			);
			toast.success(res.data.message);
		} catch (error) {
			console.log(error);
