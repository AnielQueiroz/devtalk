		...
		<div className="h-full absolute w-full md:static left-0 top-0 flex flex-col z-[800]">
			{selectedImg && (
				...
				>
					...
					<button
						type="button"
						className="absolute bg-primary rounded-lg p-2 hover:opacity-80 top-4 right-4 text-white text-2xl"
						onClick={() => setSelectedImg(null)}
					>
						<X />
					</button>
			...
			<div className="overflow-y-auto content-end flex-1 p-4 space-y-4 bg-base-100">
				{messages.map((msg) => (
					<div
						key={msg._id}
						className={`chat ${
